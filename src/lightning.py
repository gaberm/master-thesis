...
class XcopaModel(LightningModule):
    def __init__(self, config, seed):
        super().__init__()
    ...
    def training_step(self, batch, batch_idx):
        ...
        for ds_name, batch in clf_batches.items():
            if ds_name == "copa" or "xstorycloze":
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for ds_name, batch in clf_batches.items():
            if ds_name == "copa":
                ...
                self.copa_samples += len(new_labels)
            else:
                dim1 = int(len(batch["logits"]) / 3)
                probs = batch["logits"].view(dim1, 3).softmax(dim=1)
                new_labels = batch["labels"].view(dim1, 3).argmax(dim=1)
                self.pred_metric_multiclass.update(probs, new_labels)
                self.siqa_samples += len(new_labels)
    
    def on_validation_epoch_end(self):
        total_samples = self.copa_samples + self.siqa_samples
        val_score = (
            self.pred_metric_binary.compute() * self.copa_samples + 
            self.pred_metric_multiclass.compute() * self.siqa_samples
        ) / total_samples
        self.log(f"{self.pred_metric_name}", val_score, prog_bar=True, sync_dist=True)
    ...
    def set_temperature(self, val_loader):
        ...
        def eval():
            ...
            if "xcopa" in self.exp_name:
                ...
                loss = ce_loss(torch.cat(logits) / self.temperature, torch.cat(labels))
            loss.backward()
            return loss
        optimizer.step(eval)
...
class XStoryClozeModel(LightningModule):
    def __init__(self, config, seed):
        super().__init__()
        self.encoder, self.classifier = load_model(config)
        self.pred_metric_binary = load_metric(config, "pred", 2)
    ...
    def training_step(self, batch, batch_idx):
        ...
        #     try:
        #         ds_batch = {k: v.to(self.device) for k, v in ds_batch.items()}
        #         outputs = self.encoder(ds_batch["input_ids"], ds_batch["attention_mask"])
        #         clf_batches[ds_name] = {
        #             "logits": self.classifier(outputs.pooler_output),
        #             "labels": ds_batch["labels"],
        #         }
        ...
        # loss = torch.tensor(0.0, device="cuda")
        # for ds_name, batch in clf_batches.items():
        #     if ds_name == "storycloze":
        #         dim1 = int(len(batch["logits"]) / 2)
        #         new_logits = batch["logits"].view(dim1, 2)
        #         new_labels = batch["labels"].view(dim1, 2).argmax(dim=1)
        #     else:
        #         dim1 = int(len(batch["logits"]) / 4)
        #         new_logits = batch["logits"].view(dim1, 4)
        #         new_labels = batch["labels"].view(dim1, 4).argmax(dim=1)
        #     loss += self.ce_loss(new_logits, new_labels)
        # self.log("train_loss", loss)
    ...
    def on_validation_epoch_end(self):
        val_score = self.pred_metric_binary.compute()
        self.log(f"{self.pred_metric_name}", val_score, prog_bar=True, sync_dist=True)
        self.pred_metric_binary.reset()
    ...
    def set_temperature(self, val_loader):
        ...
        def eval():
            optimizer.zero_grad()
            loss = ce_loss(torch.cat(logit_lst) / self.temperature, torch.cat(label_lst))
            loss.backward()
            return loss
        optimizer.step(eval)
...
def load_l_model(config, seed):
    ...
    if config.model.load_ckpt:
        ...
        if config.model.ckpt_avg == "none":
            if config.dataset.name == "xstorycloze":
                l_model = XStoryClozeModel.load_from_checkpoint(best_ckpt, map_location=device)
    ...
    # train: load model from scratch
        ...
        if config.dataset.name == "xstorycloze":
            return XStoryClozeModel(config, seed)
        elif config.dataset.name == "xcopa":    
            return XcopaModel(config, seed)
        else:
